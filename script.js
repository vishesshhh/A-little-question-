...
function secret(el){
  ...
  function update(){
    ...
    display.innerHTML=words.map((word,wi)=>{
      let s=''; if(wi>0)s+='<span class="word-space"></span>';
      s+='<span class="word">';
      for(let j=0;j<word.length;j++,cursor++) s+=`<span class="${cursor<typed.length?'filled':''}">${cursor<typed.length?SECRET[cursor]:'_'}</span>`;
      s+='</span>'; return s;
    }).join('');
    const exp=expected();
