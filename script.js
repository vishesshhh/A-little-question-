...
function hidden(el){
  el.innerHTML=`<section class="screen hidden"><div class="hidden-card"><h2>Now there's something hidden...</h2><p class="redline">Can you figure it out? 👀</p><p class="hint">Use the keyboard t[...]`
...
function secret(el){
  ...
  function update(){
    ...
    display.innerHTML=words.map((word,wi)=>{
      // Skip the real space between words, but never render it as a
      // character that can cause a line break inside a word.
      if(wi>0) cursor++;
